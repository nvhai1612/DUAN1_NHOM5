...
//            return check > 0;
    ...
    public ArrayList<HoaDonDTO> getListFormDB() {
        ...
        try (Connection con = connection.getConnection();
                ...
                PreparedStatement ps = con.prepareStatement("select MaHD,TENKH,TENNV,h.TRANGTHAIHD,MASP,TENSP,SOLUONG,hc.DONGIA,NHANVIEN.SDT,NHANVIEN.DIACHI,h.NGAYTHANHTOAN from hoadonct hc join hoadon h on h.id = hc.IDHD join SANPHAMCHITIET sc on sc.id = hc.IDSPCT join sanpham s on s.id = sc.IDSP join NHANVIEN on NHANVIEN.ID=h.IDNV join  KHACHHANG on KHACHHANG.ID=h.IDKH")){
